...
import { coordinate2Position, hasInLine, position2Coordinate } from './position.js';
import { getAllShapesOfPoint, getShapeFast, isFive, isFour, shapes } from './shape.js';
...
export default class Evaluate {
    ...
    getPoints(role, depth, vct, vcf) {
        ...
        if (config.onlyInLine && this.history.length >= config.inlineCount) {
            ...
            if (pointsInLine) {
                performance.getPointsTime += Date.now() - start;
                return pointsInLine;
            }
        }
    ...
    updatePoint(x, y) {
        ...
        performance.updateTime += Date.now() - start;
    }
    ...
    getMoves(role, depth, onlyThree = false, onlyFour = false) {
        const moves = Array.from(this._getMoves(role, depth, onlyThree, onlyFour)).map((move) => [Math.floor(move / this.size), move % this.size]);
        return moves;
    }
